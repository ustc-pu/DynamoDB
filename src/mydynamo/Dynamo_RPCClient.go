...
func (dynamoClient *RPCClient) PutToSingleNode(value PutArgs) bool {
	var result bool
	if dynamoClient.rpcConn == nil {
		return false
	}
	err := dynamoClient.rpcConn.Call("MyDynamo.PutToSingleNode", value, &result)
	if err != nil {
		log.Println(err)
		return false
	}
...
//Gets a value from a server.
func (dynamoClient *RPCClient) GetFromSingleNode(key string) *DynamoResult {
	var result DynamoResult
	if dynamoClient.rpcConn == nil {
		return nil
	}
	err := dynamoClient.rpcConn.Call("MyDynamo.GetFromSingleNode", key, &result)
	if err != nil {
		log.Println(err)
		return nil
	}
